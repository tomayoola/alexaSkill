...
describe('api api-wrapper testing', () => {
    ...
    describe('# get-skill-credentials', () => {
        it ('| will get the skill credentials', () => {
            apiWrapper.callGetSkillCredentials('skill-id', TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);

            let arg = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(arg.method).equal('GET');
            expect(arg.url.includes('/skills/skill-id/credentials')).equal(true);
        });
    });
